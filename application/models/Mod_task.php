...
class Mod_task extends CI_Model
{
    ...
    public function e_2_1_pdf($area = '')
    {
        ...
        if (!empty($res)) {
            for ($i = 0; $i < count($res); $i++) {
                # code...
                $member_unit = $this->db->where('year', $_SESSION['year'])->where('member_code', $res[$i]['job_code'])->get('staff_member')->row_array();
                $arr[] = array(
    ...
    public function e_2_1($area = '', $part)
    {
        ...
        if (!empty($res)) {
            for ($i = 0; $i < count($res); $i++) {
                # code...
                $member = $this->db->where('year', $_SESSION['year'])->where('member_code', $res[$i]['job_code'])->get('staff_member')->row_array();
        ...
        if (!empty($res)) {
            ...
            for ($i = 0; $i < count($res); $i++) {
                ...
                for ($d = 0; $d < count($do_date); $d++) { 
                    ...
                    $member = $this->db->where('year', $_SESSION['year'])->where('job_code', $res[$i]['job_code'])->get('district_task')->row_array();
    ...
    public function get_all_assign_member_list()
    {
        ...
        for ($i = 0; $i < count($res); $i++) {
            # code...
            $member_unit = $this->db->where('year', $this->session->userdata('year'))->where('member_code', $res[$i]['job_code'])->select('member_unit')->get('staff_member')->row_array();
            if ($res[$i]['job_code'] != "") {
                $arr[] = array(
                    'job_code' => $res[$i]['job_code'],
                    'job' => $res[$i]['job'],
        ...
        for ($i = 0; $i < count($sub); $i++) {
            # code...
            $supervisor_1 = $this->db->where('year', $this->session->userdata('year'))->where('member_code', $sub[$i]['supervisor_1_code'])->get('staff_member')->row_array();
        ...
        for ($i = 0; $i < count($sub); $i++) {
            # code...
            $supervisor_2 = $this->db->where('year', $this->session->userdata('year'))->where('member_code', $sub[$i]['supervisor_2_code'])->get('staff_member')->row_array();
        ...
        for ($i = 0; $i < count($trial_staff); $i++) {
            # code...
            $trial_staff_member = $this->db->where('year', $this->session->userdata('year'))->where('member_code', $trial_staff[$i]['trial_staff_code'])->get('staff_member')->row_array();
        ...
        for ($i = 0; $i < count($patrol); $i++) {
            # code...
            $patrol_member = $this->db->where('year', $this->session->userdata('year'))->where('member_code', $patrol[$i]['patrol_staff_code'])->get('staff_member')->row_array();
    ...
    public function update_once($sn, $data)
    {
        ...
        $res = $this->db->where('sn', $sn)->get('district_task')->row_array();
        $staff = $this->db->where('year', $this->session->userdata('year'))->where('member_code', trim($res['job_code']))->get('staff_member')->row_array();
        $fee = $this->db->where('year', $this->session->userdata('year'))->get('exam_fees')->row_array();
        // print_r($staff);
        if ($staff['order_meal'] == 'Y') {

            $lunch = array(
                'lunch_price' => $fee['lunch_fee'],
                'lunch_total' => $fee['lunch_fee'] * $res['day_count'],
                'total' => $fee['lunch_fee'] * $res['day_count'] + $res['salary_total'],
                'meal' => $staff['meal'],
                'order_meal' => $staff['order_meal'],
            );
            $this->db->where('sn', $sn);
            $this->db->update('district_task', $lunch);
        } else {
            if ($staff['order_meal'] == '') {
                $staff['meal'] = '自備';
                $staff['order_meal'] = 'N';

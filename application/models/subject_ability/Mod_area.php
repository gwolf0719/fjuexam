...
/**
    ...
    function check_a1(){
        ...
        // A1
        $this->db->where('year',$this->session->userdata('year'));
        $count1 = $this->db->count_all_results('ability_exam_area');
        if($count1>0){$count1=1;}
        // A2
        $this->db->where('year',$this->session->userdata('year'));
        $count2 = $this->db->count_all_results('ability_school_unit');
        if($count2>0){$count2=1;}
        // A3
        $this->db->where('year',$this->session->userdata('year'));
        $count3 = $this->db->count_all_results('ability_staff_member');
        if($count3>0){$count3=1;}
        // A4
        $this->db->where('year',$this->session->userdata('year'));
        $count4 = $this->db->count_all_results('ability_district_task');

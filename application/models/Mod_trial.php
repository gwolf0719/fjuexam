...
class Mod_trial extends CI_Model
{
    ...
    public function update_once($sn, $data)
    {
        ...
        $res = $this->db->where('sn', $sn)->get('trial_assign')->row_array();
        $total = $res['first_member_section_salary_total'] - $res['first_member_section_lunch_total'];
        $total2 = $res['second_member_section_salary_total'] - $res['second_member_section_lunch_total'];
        $new_total = array(
    ...
    public function update_trial($sn, $data)
    {
        ...
        $new = array(
            'salary_total' => $res['count'] * $res['salary'],
            'total' => $res['count'] * $res['salary'] + $res['lunch_total'],
        );
        $this->db->where('sn', $sn);
        $this->db->update('trial_staff', $new);

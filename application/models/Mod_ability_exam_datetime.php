...
class Mod_ability_exam_datetime extends CI_Model
{
/**
     * 由試場起迄號得知使用日期
     * start 開始考場 end 結束考場
    ...
    public function get_once_day_section_test($res)
    {
        ...
        for ($i=0; $i < count($res); $i++) {
            ...
            $data = $this->db->where($where)->get('ability_exam_area')->row_array();
        }
        return $data;
    }
    ...
    public function get_once_course($res)
    {
        for ($i=0; $i < count($res); $i++) {
            ...
            $data = $this->db->where('field', $res[$i]['field'])->get('ability_exam_area')->row_array();
        }
        // print_r($data);
        return $data;
    }

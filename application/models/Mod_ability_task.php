...
class Mod_ability_task extends CI_Model
{
    public function import($area)
    {
        ...
        $this->db->insert_batch('ability_district_task', $area);
    }
    ...
    public function import_1($area_1)
    {
        ...
        $this->db->delete('ability_district_task');
        $this->db->insert_batch('ability_district_task', $area_1);
    }
    ...
    public function import_2($area_2)
    {
        ...
        $this->db->delete('ability_district_task');
        $this->db->insert_batch('ability_district_task', $area_2);
    }
    ...
    public function chk_once($job_code)
    {
        ...
        if ($this->db->count_all_results('ability_district_task') == 0) {
            return false;
        } else {
            return true;
        }
    ...
    public function e_2_1_pdf($area = '')
    {
        ...
        if (!empty($res)) {
            for ($i=0; $i < count($res); $i++) {
                # code...
                $member_unit = $this->db->where('member_code', $res[$i]['job_code'])->get('ability_staff_member')->row_array();
                $arr[] = array(
    ...
    public function e_2_1($area = '',$part)
    {
        ...
        if (!empty($res)) {
            for ($i=0; $i < count($res); $i++) {
                # code...
                $member = $this->db->where('member_code', $res[$i]['job_code'])->get('ability_staff_member')->row_array();
        ...
        if (!empty($res)) {
            ...
            for ($i=0; $i < count($res); $i++) {
                ...
                for ($d=0; $d < count($do_date); $d++) { 
                    ...
                    $member = $this->db->where('job_code', $res[$i]['job_code'])->get('ability_district_task')->row_array();
    ...
    public function get_member_veg_count($area = '')
    {
        ...
        if (!empty($res)) {
            ...
            for ($i=0; $i < count($res); $i++) {
                ...
                $veg = $this->db->get('ability_staff_member')->row_array();
    ...
    public function get_member_meat_count($area = '')
    {
        ...
        if (!empty($res)) {
            ...
            for ($i=0; $i < count($res); $i++) {
                ...
                $meat = $this->db->get('ability_staff_member')->row_array();
    ...
    public function get_sign_list($area = '')
    {
        ...
        $this->db->where('ability_trial_assign.supervisor_1_code !=',"");
        $this->db->where('ability_trial_assign.supervisor_2_code !=',"");
        $year = $this->session->userdata('year');
        $sub = $this->db->get()->result_array();

        //取出管卷人員
        $this->db->where('year', $this->session->userdata('year'));
        $trial_staff = $this->db->get('ability_trial_staff')->result_array();    
        
        //取出巡場人員

        $this->db->where('year', $this->session->userdata('year'));

        $patrol = $this->db->get('ability_patrol_staff')->result_array();        

        if (!empty($res)) {
            for ($i=0; $i < count($res); $i++) {
                # code...
                $member = $this->db->where('year', $this->session->userdata('year'))->where('member_code', $res[$i]['job_code'])->get('ability_staff_member')->result_array();

                for ($m=0; $m < count($member); $m++) { 
                    # code...
                    $unit = $this->db->where('year', $this->session->userdata('year'))->where('unit', $member[$m]['unit'])->where('member_code',$member[$m]['member_code'])->get('ability_staff_member')->row_array();
                    $arr[$unit['unit']][] = array(
            ...
            for ($i=0; $i < count($sub); $i++) {
                # code...
                $member = $this->db->where('year', $this->session->userdata('year'))->where('member_code', $sub[$i]['supervisor_2_code'])->get('ability_staff_member')->row_array();

                $unit = $this->db->where('year', $this->session->userdata('year'))->where('unit', $member['unit'])->where('member_code',$member['member_code'])->get('ability_staff_member')->row_array();

                $member1 = $this->db->where('year', $this->session->userdata('year'))->where('member_code', $sub[$i]['supervisor_1_code'])->get('ability_staff_member')->row_array();
                $unit1 = $this->db->where('year', $this->session->userdata('year'))->where('unit', $member1['unit'])->where('member_code',$member1['member_code'])->get('ability_staff_member')->row_array();
                $arr[$unit['unit']][] = array(
            ...
            for ($i=0; $i < count($trial_staff); $i++) {
                # code...
                $member = $this->db->where('year', $this->session->userdata('year'))->where('member_code', $trial_staff[$i]['trial_staff_code'])->get('ability_staff_member')->row_array();
                $unit = $this->db->where('year', $this->session->userdata('year'))->where('unit', $member['unit'])->where('member_code',$member['member_code'])->get('ability_staff_member')->row_array();
                $arr[$unit['unit']][] = array(
                    'member_code'=>$member['member_code'],
                    'member_name'=>$member['member_name'],
            ...
            for ($i=0; $i < count($patrol); $i++) {
                # code...
                $member = $this->db->where('year', $this->session->userdata('year'))->where('member_code', $patrol[$i]['patrol_staff_code'])->get('ability_staff_member')->row_array();
                $unit = $this->db->where('year', $this->session->userdata('year'))->where('unit', $member['unit'])->where('member_code',$member['member_code'])->get('ability_staff_member')->row_array();
                $arr[$unit['unit']][] = array(
                    'member_code'=>$member['member_code'],
                    'member_name'=>$member['member_name'],
    ...
    public function member_map($area = '')
    {
        ...
        if (!empty($res)) {
            ...
            $this->db->where('year', $this->session->userdata('year'))->delete('ability_member_map');
            $this->db->insert_batch('ability_member_map', $arr);
    ...
    public function get_member_map_list()
    {
        ...
        $res = $this->db->get('ability_member_map')->result_array();
        for ($i=0; $i < count($res); $i++) { 
            # code...
    ...
    public function get_list_for_pdf()
    {
        ...
        for ($i=0; $i < count($res); $i++) {
            # code...
            $member_unit = $this->db->where('member_code', $res[$i]['job_code'])->select('member_unit')->get('ability_staff_member')->row_array();
            if ($res[$i]['job_code'] != "") {
                $arr[] = array(
                    'job_code' => $res[$i]['job_code'],
                    'name' => $res[$i]['name'],
    ...
    public function get_all_assign_member_list()
    {
        ...
        for ($i=0; $i < count($trial_staff); $i++) {
            # code...
            $trial_staff_member = $this->db->where('member_code', $trial_staff[$i]['trial_staff_code'])->get('ability_staff_member')->row_array();
        ...
        for ($i=0; $i < count($patrol); $i++) {
            # code...
            $patrol_member = $this->db->where('member_code', $patrol[$i]['patrol_staff_code'])->get('ability_staff_member')->row_array();
    ...
    {
        return $this->db->where('sn', $sn)->get('ability_district_task')->row_array();
    }

    public function get_part_for_once($part)
    {
        return $this->db->where('area', $part)->get('ability_district_task')->row_array();
    }
    ...
    {
        $this->db->where('sn', $sn);
        $this->db->update('ability_district_task', $data);
    ...
    {
        $this->db->where('sn', $sn);
        $this->db->update('ability_district_task', $sql_data);
